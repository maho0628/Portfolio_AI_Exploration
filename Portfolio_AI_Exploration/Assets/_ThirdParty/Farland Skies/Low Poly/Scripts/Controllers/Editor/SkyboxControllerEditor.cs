...
namespace Borodar.FarlandSkies.LowPoly
{
    ...
    public class SkyboxControllerEditor : Editor
    {
        ...
        private void StarsCubemapPopup(bool enabledBefore)
        {
            ...
            if (_presets.StarsCubemaps == null || _presets.StarsCubemapNames == null)
                return;

            var cubemaps = _presets.StarsCubemaps;
            var cubemapNames = _presets.StarsCubemapNames;

            var currentCubemap = _starsCubemap != null
                ? _starsCubemap.objectReferenceValue as Cubemap
                : null;
            ...
            else
            {
                if (ArrayUtility.Contains(cubemaps, currentCubemap))
                    _starsCubemap.objectReferenceValue = null;
        ...
        private void SunTexturePopup(bool enabledBefore)
        {
            ...
            if (_presets.SunTextures == null || _presets.SunTextureNames == null)
                return;

            var textures = _presets.SunTextures;
            var textureNames = _presets.SunTextureNames;

            var currentTexture = _sunTexture != null
                ? _sunTexture.objectReferenceValue as Texture2D
                : null;
            ...
            else
            {
                if (ArrayUtility.Contains(textures, currentTexture))
                    _sunTexture.objectReferenceValue = null;
        ...
        private void MoonTexturePopup(bool enabledBefore)
        {
            ...
            if (_presets.MoonTextures == null || _presets.MoonTextureNames == null)
                return;

            var textures = _presets.MoonTextures;
            var textureNames = _presets.MoonTextureNames;

            var currentTexture = _moonTexture != null
                ? _moonTexture.objectReferenceValue as Texture2D
                : null;
        ...
        private void CloudsCubemapPopup(bool enabledBefore)
        {
            ...
            if (_presets.CloudsCubemaps == null || _presets.CloudsCubemapNames == null)
                return;

            var cubemaps = _presets.CloudsCubemaps;
            var cubemapNames = _presets.CloudsCubemapNames;

            var currentCubemap = _cloudsCubemap != null
                ? _cloudsCubemap.objectReferenceValue as Cubemap
                : null;
